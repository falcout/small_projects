...
class UI:
    ...
    def input(self):
        ...
        if self.state == 'general':
            self.general_index['row'] = (self.general_index['row'] + keys[pygame.K_DOWN] - keys[pygame.K_UP]) % self.rows
            self.general_index['col'] = (self.general_index['col'] + keys[pygame.K_RIGHT] - keys[pygame.K_LEFT]) % self.cols
            if keys[pygame.K_SPACE]:
                self.state = self.general_options[self.general_index['col'] + self.general_index['row'] * 2]
        
        elif self.state == 'attack':
            self.attack_index['row'] = (self.attack_index['row'] + keys[pygame.K_DOWN] - keys[pygame.K_UP]) % self.rows
            self.attack_index['col'] = (self.attack_index['col'] + keys[pygame.K_RIGHT] - keys[pygame.K_LEFT]) % self.cols
            if keys[pygame.K_SPACE]:
            ...
            if self.available_monsters:
                self.switch_index = (self.switch_index + keys[pygame.K_DOWN] - keys[pygame.K_UP]) % len(self.available_monsters)
                if keys[pygame.K_SPACE]:
    ...
    def switch(self):
        ...
        # menu
        v_offset = 0 if self.switch_index < self.visible_monsters else (self.visible_monsters - self.switch_index - 1) * rect.height / self.visible_monsters
        for i in range(len(self.available_monsters)):
            ...
            simple_rect = simple_surf.get_frect(center=(x-(rect.width/4), y))
...
class OpponentUI:
    ...
    def draw(self):
        ...
        # health bar
        health_rect = pygame.FRect(name_rect.left, name_rect.bottom + 10, rect.width * 0.9, 20)
        ratio = health_rect.width / self.opponent.max_health
        progress_rect = pygame.FRect(health_rect.topleft, (self.opponent.health * ratio, health_rect.height))
        pygame.draw.rect(self.screen, COLORS['gray'], health_rect)
        pygame.draw.rect(self.screen, COLORS['red'], progress_rect)
